...
import math
from algo.network import Discrimator
...
class Runner:
    ...
    def __init__(self, args, env, meta_policy, opponent, buffer, logger, device, 
        ...
        self.discrim_net = Discrimator([4, 30, 30], 14, action_space.n)
        self.discrim_criterion = torch.nn.BCELoss
        self.discrim_optim = torch.optim.Adam(self.discrim_net.parameters(), lr=args.d_lr)

    def update_discrim(self, data, update_itr):

        obs = torch.from_numpy(np.stack(data.obs)).to(dtype).to(self.device)
        actions = torch.from_numpy(np.stack(data.action)).to(dtype).to(self.device)
        infos = torch.from_numpy(np.stack(data.info)).to(dtype).to(self.device)
        
        for _ in range(update_itr):
            expert_state_actions = torch.from_numpy(self.expert_traj).to(type).to(self.device)
            g_o = self.discrim_net(obs, actions, infos)
            e_o =  self.discrim_net(expert_state_actions)
            self.discrim_optim.zero_grad()
            loss = self.discrim_criterion(g_o, torch.ones((obs.action[0], 1), device=self.device))+\
                    self.discrim_criterion(e_o, torch.zeros((self.expert_traj.shape[0],1), device=self.device))
            loss.backward()
    ...
    def run_low_policy(self, policy, ob, agent_idx):
        ...
        while not done:
            ...
            a = policy.act(torch.as_tensor(obs[newaxis], dtype=dtype, device=self.device), True)
        ...
        for epoch in range(epochs):
            ...
            while(step < self.local_steps_per_epoch):
                ...
                if who_is_throwing == self.agent_idx:
                    a, v, logp = self.meta_policy.step(torch.as_tensor(obs_ctrl[newaxis][newaxis], dtype=dtype, device=self.device),
                                                  torch.as_tensor(info_ctrl[newaxis], dtype=dtype, device=self.device))
                    step += 1
                    ...
                    a = self.opponent.act(torch.as_tensor(obs_oppo[newaxis][newaxis], dtype=dtype, device=self.device),
                                        torch.as_tensor(info_oppo[newaxis], dtype=dtype, device=self.device))
                    low_policy = self.get_low_policy(a)
                ...
                if done or epoch_ended:
                    ...
                    if epoch_ended:
                        _, v, _ = self.meta_policy.step(torch.as_tensor(o['obs'][self.agent_idx][-1][newaxis][newaxis], dtype=dtype, device=self.device),
                                                   torch.as_tensor(o['info'][self.agent_idx][newaxis], dtype=dtype, device=self.device) )
                    else:
                        v = 0
